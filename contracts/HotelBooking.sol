...
contract HotelBooking is ReentrancyGuard, Ownable {
     ...
     */
    function payForBooking(uint256 _bookingId) 
        public 
        payable 
        ...
        nonReentrant
    {
        ...
        require(!booking.isCancelled, "Booking has been cancelled");
        require(!booking.isPaid, "Booking has already been paid");
        require(msg.value >= booking.totalPrice, "Insufficient payment amount");
        
        booking.isPaid = true;
        
        // Transfer payment to hotel owner
        address hotelOwner = hotels[booking.hotelId].hotelOwner;
        ...
        onlyGuest(_bookingId) 
    {
        ...
        require(!booking.isCancelled, "Booking has already been cancelled");
        require(!booking.isPaid, "Paid bookings cannot be cancelled");
    ...
    /**
     * @dev Get all room IDs for a hotel
     * @param _hotelId Hotel ID
    ...
    /**
     * @dev Get all booking IDs for a guest
    ...
    /**
     * @dev Check if a room is available for the given date range
     * @param _roomId Room ID
     * @param _checkInDate Check-in date (Unix timestamp)
     * @param _checkOutDate Check-out date (Unix timestamp)
     * @return true if the room is available
     */
    function isRoomAvailable(uint256 _roomId, uint256 _checkInDate, uint256 _checkOutDate) 
        public 
        view 
        returns (bool) 
    {
        if (!rooms[_roomId].isAvailable) {
            return false;
        }
        
        // Check for overlapping bookings (in a real implementation, this would be more complex)
        return true;
    }
} 